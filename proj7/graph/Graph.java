...
public class Graph<V> {
   ...
   public boolean addEdgeBetweenVertices(V source, V dest, int cost) {
      ...
      for (adjacentVertex adjVer : adjacencyMap.get(source)) {
   ...
   public boolean contractEdgeBetweenVertices(V vertex1, V vertex2) {
      ...
      //obtain list of all vertex1 adjacency data
      List<adjacentVertex> vertex1List = adjacencyMap.get(vertex1);
      vertex1List.addAll(adjacencyMap.get(vertex2));
      
      //make a copy to do independent double iteration
      List<adjacentVertex> vertex1ListCopy = new ArrayList<adjacentVertex>(vertex1List);
      ...
      for (adjacentVertex adjVer1 : vertex1List) {
         for (adjacentVertex adjVer2 : vertex1ListCopy) {
            if (adjVer1.vertex.equals(adjVer2.vertex) && !adjVer1.equals(adjVer2)) {
               //same vertex, but different weight
               // remove the smaller cost vertex
               if (adjVer1.weight > adjVer2.weight) {
                  vertex1List.remove(adjVer2);
               } else {
                  vertex1List.remove(adjVer1);
               }
            }
         }
      }
